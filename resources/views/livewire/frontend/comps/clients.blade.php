<div class="container-fluid relative">


    <div class="clients__container container mt-[100px] mb-[100px] mx-auto">


        <h4 class="text-[32px] font-semibold text-secondary-500 text-center">Trusted by corporate national and
            international companies</h4>


        {{-- clients logos --}}
        <div class="clients__logos flex justify-between items-center gap-10 mt-[83px] mb-[100px]">
            <a href="#"><img src="{{ asset('frontend/images/clients/clients1.png') }}" width="auto"
                    alt="Client Logo 1"></a>
            <a href="#"><img src="{{ asset('frontend/images/clients/clients2.png') }}" alt="Client Logo 2"></a>
            <a href="#"><img src="{{ asset('frontend/images/clients/clients3.png') }}" width="auto"
                    class="w-auto" alt="Client Logo 3"></a>
            <a href="#"><img src="{{ asset('frontend/images/clients/clients4.png') }}" alt="Client Logo 4"></a>
            <a href="#"><img src="{{ asset('frontend/images/clients/clients2.png') }}" alt="Client Logo 5"></a>
            <a href="#"><img src="{{ asset('frontend/images/clients/clients3.png') }}" alt="Client Logo 5"></a>
        </div>

        <div class="container content-center">
            <h2 class="text-h2 text-black">Built for complete visual solutions</h2>
            <p class="text-p1 tet-black max-w-[612px] mt-[15px] mb-[28px]">From concept to completion, we craft
                memorable visuals that transform business identities.</p>

            <a href="#" class="button bg-primary-default text-secondary-default hover:bg-primary-700">Get To Know
                Us <svg width="13" height="12" viewBox="0 0 13 12" fill="none"
                    xmlns="http://www.w3.org/2000/svg">
                    <path
                        d="M10.2039 4.91091L6.65191 1.35887C6.50386 1.21082 6.42815 1.02474 6.42476 0.800624C6.42138 0.576684 6.49709 0.387218 6.65191 0.232223C6.80672 0.0774073 6.9945 -7.52383e-07 7.21523 -7.24128e-07C7.43579 -6.95895e-07 7.62347 0.0774074 7.77829 0.232224L12.5823 5.03624C12.7756 5.22954 12.8723 5.455 12.8723 5.71261C12.8723 5.97022 12.7756 6.19567 12.5823 6.38897L7.77829 11.193C7.63042 11.3409 7.44443 11.4166 7.22031 11.4201C6.99619 11.4235 6.80672 11.3478 6.65191 11.193C6.49709 11.038 6.41968 10.8502 6.41968 10.6297C6.41968 10.4091 6.49709 10.2213 6.65191 10.0663L10.2039 6.5143L0.801677 6.5143C0.574174 6.5143 0.383637 6.43751 0.230068 6.28395C0.0766773 6.13055 -1.71661e-05 5.94011 -1.71661e-05 5.7126C-1.71661e-05 5.4851 0.0766773 5.29465 0.230068 5.14126C0.383637 4.98769 0.574174 4.91091 0.801677 4.91091L10.2039 4.91091Z"
                        fill="#090D17" />
                </svg>
            </a>
        </div>


    </div>


    {{-- Left svg image --}}
    <svg class="absolute left-0 bottom-[-75%] z-0" width="743" height="661" viewBox="0 0 743 661" fill="none"
        xmlns="http://www.w3.org/2000/svg">
        <path d="M168.328 660.455H476.342L609.468 527.309H168.328V660.455Z" fill="url(#paint0_linear_17_161)" />
        <path
            d="M742.755 394.323C742.755 322.236 684.192 263.734 612.029 263.734H296.494H263.692C191.529 263.734 132.966 205.233 132.966 133.146L0 266.132C0 338.219 58.5627 396.721 130.726 396.721H403.539H479.062C551.226 396.721 609.788 455.222 609.788 527.309L742.915 394.323H742.755Z"
            fill="url(#paint1_linear_17_161)" />
        <path
            d="M446.261 132.986C512.824 132.986 567.066 187.172 567.066 253.665H579.387C579.387 187.172 633.629 132.986 700.193 132.986V120.678C633.629 120.678 579.387 66.493 579.387 0H567.066C567.066 66.493 512.824 120.678 446.261 120.678V0H266.412L133.286 132.986H446.261Z"
            fill="url(#paint2_linear_17_161)" />
        <defs>
            <linearGradient id="paint0_linear_17_161" x1="56.6315" y1="674.795" x2="206.321" y2="420.238"
                gradientUnits="userSpaceOnUse">
                <stop stop-color="#D8F2FC" />
                <stop offset="1" stop-color="#D8F2FC" stop-opacity="0" />
            </linearGradient>
            <linearGradient id="paint1_linear_17_161" x1="-188.105" y1="569.759" x2="318.7" y2="79.4748"
                gradientUnits="userSpaceOnUse">
                <stop stop-color="#D8F2FC" />
                <stop offset="1" stop-color="#D8F2FC" stop-opacity="0" />
            </linearGradient>
            <linearGradient id="paint2_linear_17_161" x1="-10.254" y1="280.983" x2="313.033" y2="-89.8534"
                gradientUnits="userSpaceOnUse">
                <stop stop-color="#D8F2FC" />
                <stop offset="1" stop-color="#D8F2FC" stop-opacity="0" />
            </linearGradient>
        </defs>
    </svg>


    {{-- Right SVG image  --}}
    <svg class="absolute bottom-[-45%] right-0 z-0" width="743" height="528" viewBox="0 0 743 528" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path
            d="M742.755 394.323C742.755 322.236 684.192 263.734 612.029 263.734H296.494H263.692C191.529 263.734 132.966 205.233 132.966 133.146L0 266.132C0 338.219 58.5627 396.721 130.726 396.721H403.539H479.062C551.226 396.721 609.788 455.222 609.788 527.309L742.915 394.323H742.755Z"
            fill="url(#paint0_linear_1_2)" />
        <path
            d="M446.261 132.986C512.824 132.986 567.066 187.172 567.066 253.665H579.387C579.387 187.172 633.629 132.986 700.193 132.986V120.678C633.629 120.678 579.387 66.493 579.387 0H567.066C567.066 66.493 512.824 120.678 446.261 120.678V0H266.412L133.286 132.986H446.261Z"
            fill="url(#paint1_linear_1_2)" />
        <defs>
            <linearGradient id="paint0_linear_1_2" x1="1051.03" y1="561.567" x2="456.429" y2="-13.6469"
                gradientUnits="userSpaceOnUse">
                <stop stop-color="#D8F2FC" />
                <stop offset="1" stop-color="#D8F2FC" stop-opacity="0" />
            </linearGradient>
            <linearGradient id="paint1_linear_1_2" x1="935.308" y1="275.711" x2="556.02" y2="-159.364"
                gradientUnits="userSpaceOnUse">
                <stop stop-color="#D8F2FC" />
                <stop offset="1" stop-color="#D8F2FC" stop-opacity="0" />
            </linearGradient>
        </defs>
    </svg>





</div>
